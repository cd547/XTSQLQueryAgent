...
> 版本：V1 · 核对日期：2026-08-25（§13 修正：fileIds 走 selectedFileIds 勾选子集、白名单正则含短横线、多文件批量上传、vision 守卫、图片回显本地缓存链路 A5-A10）
...
1. **reasoning 剥离**：无 tool_calls 的 assistant 剥离 `reasoning_content`（DeepSeek 协议要求，见 §2.2）；messages **直接作为请求体**（2026-08-25 起不再折叠，见下）；
2. **prune 已禁用**（F22，2026-08-18）：tools 数组保持字节稳定以命中 prefix cache；`validate_sql_fields` 永不剪枝。`get_call_history` 不在 LLM_TOOLS 中（LLM 不可见）。
3. **get_call_history 合成注入**（F23，CC/RA 双路径一致）：每轮 push assistant 消息前，若本轮实际调用了非 get_call_history 工具且未误调该工具，则在 tool_calls 头部 unshift 常量 id 的 `synthetic_get_call_history`；执行阶段由拦截器返回基于 `reg.callHistory` 的稳定 JSON（含 `_instruction` 提示）。连续两轮 LLM 仅调 get_call_history → 强制终止（gchLoopCount 熔断）。

> ★ **折叠机制已移除**（2026-08-25 方案 B）：原 `compactConsumedToolResults` 将"已消费历史区"的 `get_sliced_index` 结果替换为去掉 `related_tables` 的 Compact 卡片。实测该折叠仅省 ~970 tokens/43 表（Compact 版与完整版唯一差异就是 related_tables 一行），却导致模型在 R1 之后及 user_choice 续问场景看不到关联表 → 无法批量规划 `get_table_schema` → 渐进式分批多花一整轮 LLM 往返（~10s + 千级 completion tokens）；且历史中段字节突变破坏 prefix cache。移除后 messages 纯追加、字节稳定，`related_tables` 全程可见。旧版 checklistMsg 机制（F23 已从双路径移除）同样不复存在。

> ★ 2026-08-25 性能修复：`saveMessagesToDb` 的 `message_tokens` 改用调用方传入的 API 权威 `usage.prompt_tokens`（runSqlAgent 在流式解析中记录 `latestPromptTokens`），不再每轮对本会话全量上下文跑本地 BPE（旧实现同步阻塞事件循环，实测 100KB ≈1.7s / 轮）。`llm_messages.message_tokens` 口径从"本地 BPE 累计"变为"最近一轮请求的 prompt_tokens"，前端"上下文长度"展示语义不变且更准。未传值的调用方回落旧 BPE 计算。

请求参数关键字段：`model`、`messages`、`temperature:0`、`stream:true`、`stream_options.include_usage`、`tools`（固定 5 个）、`thinking:{type:'enabled'}` + 顶层 `reasoning_effort`（DeepSeek 特有，三档 low/medium/high 由前端思考模式控件决定）。
...
- 启动日志双写：控制台 + `logs/electron-startup-<年-月>.log`（按月归档，每次启动追加会话分隔线），失败时 splash 提供“打开日志/复制日志”；
...
- 状态：
  - `uploadedFiles: [{id, filename, bytes, created_at, purpose, expires_at?}]` — 全局已上传（跨会话共享）
  - `uploadingFiles: [{tempId, name, size, progress, controller}]` — 正在上传
  - `filesConfig: {allowedTypes, maxSizeMiB, expiresAfterSeconds}` — 后端配置
  - `selectedFileIds: Set<string>` — **本次发送勾选的子集**：上传成功后自动 add，用户点缩略图可 toggle，发送成功后由 App.jsx 调 `clearSelected()` 清空
  - `blobUrlMapRef: Map<file_id, objectURL>` — 缩略图/回显 blobURL 缓存；`getBlobUrl(fileId)` 懒加载本地缓存（inflight 去重），卸载时统一 revoke
- 方法：
...
- `handleSend` 的 `queryGenerateStream` body 增加 `fileIds: [...selectedFileIds]`（**仅勾选子集**，非空时才传；不是 uploadedFiles 全量）；
- meta 事件到达（请求已被后端接受）后调 `clearSelected()` 清空选中集，缩略图 picker 自动消失；请求失败则保留勾选；
- vision 守卫：选了图片但当前模型非 `deepseek-v4-flash-vision-exp` → 前端红条提示 + Modal.confirm 二次确认（确认后仅发文本），后端 `llm.js` 入口 `isVisionModel` 再兜底静默丢 + warn 日志；
...
sequenceDiagram
    ...
    U->>App: 点发送
    App->>App: handleSend → fileIds: [...selectedFileIds]（勾选子集）
...
2. **白名单清洗在 query.js 入口**：`/^file-api-[A-Za-z0-9-]+$/` 严格匹配（**必须含短横线**，DeepSeek 返回的 file_id 形如 `file-api-dc6f9a94-8a3a-...`），避免前端误传任意字符串污染 LLM message；
...
5. **多文件批量上传**：file input 支持一次选多个文件，`handleFileChange` 循环逐个调用 `uploadFile` 排队上传（ChatInput.jsx:121）；
6. **DeepSeek 共享 Key**：所有用户的已上传文件实际归属 LLM API Key，不做 per-user 隔离（与 DeepSeek 行为一致）；如要 per-user 隔离需自建中间存储；
7. **vision 守卫（前端提示 + 后端兜底）**：选了图片但当前模型非 vision → 前端红条 + Modal.confirm 二次确认（确认后仅发文本）；后端 `llm.js` 入口 `isVisionModel(model)` 对带 fileIds 的请求静默丢 + warn 日志；
8. **`buildUserMessage`** **双路径导出**：`services/llm.js` 导出 + `agentHelpers.js` import；`initMessagesForRun` 同步改造；两路径行为一致。
9. **图片回显走本地缓存，不依赖 DeepSeek content 端点**（A5→A10 演进结论）：实测 DeepSeek Files API 对 `user_data` purpose 的 `GET /files/{id}/content` **永远 404**（且仅支持 user_data 这一个 purpose）。因此上传成功后二进制立即落盘 `backend/file_cache/<file_id>`，SQLite `file_storage` 表只存**元数据**（mimetype/bytes/saved_at），**不存 BLOB、不存任何路径列**——磁盘路径由 file_id 直接推导（FILE_CACHE_DIR/<file_id>），项目挪目录 / Electron 便携模式换盘符天然有效；历史版本曾存过绝对路径 / 相对文件名 / BLOB 列，一次性迁移代码已在存量升级后删除；回显链路 `GET /files/:id/content` 读取优先级：DB 路径索引（`resolveStoragePath` 基于 FILE_CACHE_DIR 推导）→ 磁盘兜底（早期文件）→ DeepSeek（实际不可用，仅防御性保留）；删除时磁盘文件与 DB 索引同步清理。
