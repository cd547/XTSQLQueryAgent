...
| 字段 | 类型 | 作用域 | 重置 |
|---|---|---|---|
| `getDomainIndexCalled` | bool | **会话级（特例）** | 不重置：一次调用即可，新问题复用上下文中的业务域列表 |
...
- **`getDomainIndexCalled` 为会话级特例**：`get_domain_index` 返回内容少且会话内不变，一个会话调用一次即可；新问题不清空该标志（工具被剪枝 + checklist 显示 `get_domain_index:✓`），引导模型直接复用上下文中的业务域列表，避免重复调用（2026-08-13 优化）。
