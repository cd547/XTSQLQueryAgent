...
3. **prune**：从发给 LLM 的 tools 列表剪枝一次性工具（`get_domain_index` 调过后移除；`get_sliced_index` 有任意域 sliced 后移除）；`validate_sql_fields` **永不剪枝**（历史教训：Round 0 剪枝导致跨问题场景误拦）。
...
| 字段 | 类型 | 作用域 | 重置 |
|---|---|---|---|
| `getDomainIndexCalled` | bool | 会话级 | 否 |
| `slicedDomains` | Set | 会话级 | 否 |
| `tableSchema` | Set | 会话级 | 否 |
| `termConfirmed` | Set | 会话级 | 否 |
| `userChoiceAsked` | Map | 会话级 | 否 |
| `getTablesCalled` | bool | 会话级 | 否（对应工具已停用，字段保留） |
| `validateSqlFieldsCalled/Passed/ErrorCount` | bool/number | **问题级** | 每次新 user 消息 `resetPerQuestionRegistryFlags` |

判断原则：新问题到来时该值是否仍成立？成立 → 会话级不重置；不成立（如“上一问的 SQL 校验通过”）→ 问题级必须重置，否则旧 “passed” 残留会误导 LLM 跳过本轮校验（2026-07-28 严重 bug 教训）。
