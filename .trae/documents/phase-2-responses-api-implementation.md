...
- ✅ 2026-08-07：v5.15 **cached_tokens 字段入库（prefix cache 命中率分析）**：用户问"现在 messages 表里存的是哪两个 token 字段？再加 cached_tokens"。**答案**：`messages` 表（**不是** `llm_messages` 表）已有 `prompt_tokens` / `completion_tokens` / `total_tokens`（[sqlite.js:113-115](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/db/sqlite.js#L113-L115)），缺 `cached_tokens`（命中上下文缓存的输入 token 数）。**改法**：① [sqlite.js:116,169-179](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/db/sqlite.js#L116) `messages` 表 schema 加 `cached_tokens INTEGER DEFAULT 0` + 老库 `PRAGMA table_info` 检测后 `ALTER TABLE` 迁移；② [llm.js:1442](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/services/llm.js#L1442) CC path yield usage 多带 `cached_tokens: cacheHit`（cacheHit 来自 `usage.prompt_cache_hit_tokens`）；③ [responsesApi.js:247](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/services/responsesApi.js#L247) Responses path yield usage 多带 `cached_tokens: cacheHit`（来自 `usage.input_tokens_details.cached_tokens`）；④ [query.js:481-482](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/routes/query.js#L481-L482) CC path INSERT messages 表加 cached_tokens 列；⑤ [responsesApi.js:282-283](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/services/responsesApi.js#L282-L283) Responses path INSERT messages 表加 cached_tokens 列（与 CC path 1:1 对齐）。**前端使用**：在 UI 上展示 prefix cache 命中率 = `cached_tokens / prompt_tokens`（前端用 `chunk.usage.cached_tokens / chunk.usage.prompt_tokens` 实时算），后端只透传数据不存派生值。**验证**：① 4 文件语法 exit 0；② test-v515-cached-tokens 9 pass（schema 列定义 / INSERT 写入 / 命中率算法 / 迁移幂等）；③ 全回归 test-agent-helpers 23 + test-convert-input-items 40 + test-f14-dispatch-integration 4 + test-route-apimode-dispatch 21 + test-v514-apimode-persistence 11 + test-v515-cached-tokens 9 + test-schema-migration 全过 = **108+ pass / 0 fail**。
