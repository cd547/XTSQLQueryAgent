...
- ✅ 2026-08-07：v5.14 **api_mode 持久化（仅加标记，不改 messages 数组结构）**：用户问"两种 API 存的格式一样，怎么展示实际情况"。**根因**：messages 数组在两种 API 走同一个 OpenAI Chat Completions 通用结构（`{role, content, tool_calls, tool_call_id, reasoning_content}`），Responses path 在 generator L535-541 显式把 Responses API 工具调用转回通用结构才存盘（保证前端 UI 表现一致）。**改法**（与用户确认"保持现状"，仅加 api_mode 标记，不动 messages 数组结构）：① [sqlite.js:179,184-194](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/db/sqlite.js#L179) DB schema 加 `api_mode TEXT DEFAULT 'chat_completions'` 列 + 老库 `PRAGMA table_info` 检测后 `ALTER TABLE` 迁移（不丢数据）；② [llm.js:920,933-940](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/services/llm.js#L920) saveMessagesToDb 多接 `apiMode` 参数，UPDATE 用 `CASE WHEN api_mode IS NULL OR api_mode = '' THEN ? END` **保留首次模式**（保护历史会话）；③ [llm.js:957,972](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/services/llm.js#L957) loadMessagesFromDb SELECT 加 `api_mode` 字段（**关键 bug**：v5.14 第一次改漏了 SELECT 字段 → `record.api_mode` undefined → fallback 误为 chat_completions，调试发现后修），返回 `{messages, messageTokens, apiMode}`；④ [agentHelpers.js:125,131-133](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/services/agentHelpers.js#L125) saveRunState 从 ctx 取 apiMode，默认 `'responses_api'`（generator 内部 default）；⑤ [query.js:253](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/routes/query.js#L253) `/api/query/messages/:sessionId` 返回 `apiMode` 字段。**前端使用**：在会话消息详情页面（如 `?sessionId=xxx` 侧栏或独立 `/messages` 页面）展示 `API 模式: Responses API / Chat Completions`，让用户/开发者一眼看到本次会话实际走的哪条路径。**重要决策**：不动 messages 数组结构 — 改动工具调用字段名（tool_calls ↔ function_call / tool_call_id ↔ call_id）会同时影响 LLM 下轮 prompt、UI 历史展示、messages route sanitize 逻辑（llm.js:998 找 `role==='tool'` 等），改动面太大风险太高。**验证**：① 6 文件语法 exit 0；② test-v514-apimode-persistence 11 pass（覆盖 5 链路 case + 3 schema case + 1 默认值 case + FK 约束处理 + 保留首次模式语义）；③ 全回归 test-agent-helpers 23 + test-convert-input-items 40 + test-f14-dispatch-integration 4 + test-route-apimode-dispatch 21 + test-schema-migration 全过 = **99+ pass / 0 fail**。
