...
- ✅ 2026-08-07：v5.13 **Step 3 偏差修复 v8：reasoning_text 必须回传**：Round 0 成功，Round 1 失败，DeepSeek 报告 `The 'reasoning_text' in the thinking mode must be passed back to the API.`。**根因**：thinking mode 下跨轮调用必须把上一轮 `reasoning_text` 回传 input items。我之前 v5.6 改 `convertMessagesToInputItems` 时只处理了 message/function_call/function_call_output，没处理 `reasoning_content` → Round 1 没回传 Round 0 的 reasoning，DeepSeek 4xx 拒绝。**修法**：[agentHelpers.js:546-551](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/services/agentHelpers.js#L546-L551) assistant 消息处理开头先看 `reasoning_content`，非空则 push `{type:'reasoning', content:[{type:'reasoning_text', text}]}` item。OpenAI Responses 规范字段：reasoning item 用 `content` 数组包装 `reasoning_text` 块。**验证**：① 语法 exit 0；② test-convert-input-items 30 → **40 pass**（新增 10 case：reasoning + content + tool_call 完整链路 / 空 reasoning_content 不生成 / 缺 reasoning_content 字段不抛错）；③ test-agent-helpers 23 pass 不变。
