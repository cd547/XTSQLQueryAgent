...
- ✅ 2026-08-07：v5.16c **缓存命中率实时显示修复（React 18 batched ref 清空竞争，**错位置**）**：用户报告"实时的时候，会话结束后没有展示出来，只有切换会话再回来时才能看到"。**当时认为的根因**：[App.jsx:1059-1060](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L1059-L1060) `setMessages(prev => ...)` 注册的 prev 回调是 **React 18 batched 异步执行**的，而 `roundUsagesRef.current = {}` 是**同步**清空 ref。**修法**：[App.jsx:1030-1042](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L1030-L1042) 在 `setMessages` **之前**同步读 ref 到本地变量 `_lastRoundUsage`。**v5.18 反思**（**真正**的根因）：v5.16c 修的是 done 块末尾的 `if (data.usage)` 兜底，但 done 事件**不带** `data.usage` 字段 — 这个块**永远**不进！**真正的根因**是 v5.16 第一版就**没**加 `else if (data.type === 'usage')` 事件分支（事件被静默丢弃，`roundUsagesRef` 永远空）。v5.16c/v5.17 修的"清空竞争"虽然是 React 18 batching 的真实问题，但**实际** ref 永远空 — 闭包读到 undefined 跟"清空竞争"无关。**教训**：① useRef + setState 组合时，**ref 清空必须在 setState 闭包内**（最安全）或**在 setState 之前**同步读 ref 到本地变量；② 但**更关键**的教训是：先**确认数据流是否存在**（用 console.log 看 ref 实际值），再分析 React 渲染机制 — **没**数据流通路的话分析闭包捕获是空对空。
- ✅ 2026-08-07：v5.17 **缓存命中率算法改为累计 0..currentRound**：用户反馈"实时显示已修好，但命中率算法只看当前这轮（最后一轮），不合理。应该从这轮对话往前数一直数到 0 之间的所有缓存命中率的总和。比如当前跑到了第 5 轮，那就要把 5，4，3，2，1，0 轮每个的缓存命中的 token 数求和再除以总 token 数（都是输入的）"。**修法**（前端 1 文件，2 处）：① [App.jsx:1030-1068](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L1030-L1068) SSE `type: 'done'` 事件路径：在 `setMessages` **之前**同步累积 `_sumCached/_sumPrompt/_sumCompletion/_sumTotal`（0.._lastRound），用累加结果作为 `_lastRoundUsage` 挂到 assistant 消息（**保留** v5.16c 的"同步捕获本地变量"模式，避免 React 18 batched 闭包读到清空后的 ref）；② [App.jsx:411-444](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L411-L444) 历史回看路径：每条 assistant 消息**按自己的 mRound** 累积 0..mRound 的 roundUsages（**关键**：每条 assistant 挂的 usage 反映"本轮为止的累计命中率" — 越往后的 assistant 越能体现 prefix cache 效果）。**公式**：`hitRate = sum_cached(0..R) / sum_prompt(0..R) * 100%`。**为什么这样设计**：① 单看当前 round 命中率在多轮对话下严重低估 prefix cache 价值（round 0 命中率 0% 算"冷启动"，但累积到 round 5 已经 80% — 体现 prefix cache 累计效果）；② 累积公式用 sum（不是 avg）— 因为分子分母是 token 数（线性量），不是百分比（无量纲），sum 后相除数学正确；③ 缺 round 时 fallback 0（向后兼容：老数据 round 全部 0，命中率不变）。**验证**：① Vite build `✓ 3464 modules transformed, built in 25.04s`；② 新增 [test-v517-hitrate-accumulation.mjs](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/test-v517-hitrate-accumulation.mjs) **20 pass / 0 fail**（单 round / 多 round 累积 / 跳号 / 空数据 / 命中率公式 / 100% / 0% / 越界 / 缺字段 fallback 共 10 case，每个 2 断言）；③ 全 backend 回归 test-agent-helpers 23 + test-convert-input-items 40 + test-f14-dispatch-integration 4 + test-route-apimode-dispatch 21 + test-v514-apimode-persistence 11 + test-v515-cached-tokens 9 = **108 pass / 0 fail**（不变）。**v5.18 反思**：v5.17 累积逻辑本身**正确**，但因为**没有** `type: 'usage'` 事件处理（v5.18 修复），done 时累积**总是** null — v5.17 累积代码**事实上**从未运行。**实操验证步骤**（v5.18 修复后）：① 第一次发 query → round 0，命中率反映 round 0 单轮；② 同一会话继续发 query → 触发多轮 LLM 调工具 → 最后一条 assistant 显示 round 0..R 累计命中率（**应该**比 round 0 单轮高）；③ hover 看明细：cached 数字会随轮数增加。
- ✅ 2026-08-07：v5.19b **缓存命中率历史回看 per-question 独立**（按"问题边界"分桶 segment-bucket）：用户报告"在一次3轮不同的会话中，每轮会话结束后上面的命中率就会发生变化"（**1 个会话问 3 个问题**）。**根因**：[App.jsx:395-407](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L395-L407) v5.16 + v5.17 的 `const roundUsages = {}; roundUsages[r] = {...}` 是**全局** + **覆盖**式构造。DB messages 顺序：user → assistant → usage (round 0) → user → assistant → usage (round 0) → user → assistant → usage (round 0)。3 个问题都 round 0 → 后插入的 Q3_usage 覆盖 Q1_usage / Q2_usage → roundUsages[0] = u_Q3 → asst1 累积 0..0 = u_Q3 的命中率（**错**！）→ 用户观察到的"上面的命中率会变化"（**切走再回来**时历史回看算的是 u_Q3）。**SSE 流式路径不受影响**：done 块末尾 `roundUsagesRef.current = {}`（[App.jsx:1118](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L1118)）保证下个问题从空开始累加。**修法**（前端 1 文件，2 处）：① [App.jsx:417-436](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L417-L436) **两遍扫描**（v5.19b 反思：assistant 消息在 DB 顺序里**在** usage 消息**之前** — 流式期间**只**创建一条 assistant，usage 是 LLM 调用结束 yield 追加的）— 第一遍按 user 切分 segments + 段内累积 usage；第二遍每条 assistant 用本段 segmentUsages 算 0..mRound 累积；② [App.jsx:438-466](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L438-L466) 构造 `assistantUsages = { [assistantDbId]: usage | undefined }` map，`loaded.map` 里直接查 `assistantUsages[m.id]` — 不再在 map 里**每个** assistant 算一次（性能 + 可读性）。**关键设计**：① 段边界用 `m.role === 'user' && i > 0`（**不**切第一段，即使它没 user）— 兼容异常场景（首条是 assistant）；② 段内同 round 的 usage 累积（**不**覆盖）— 安全 fallback（同段内同 round 不应出现多条）；③ hasAny 守卫 — 没 usage 消息时 `assistantUsages[id] = undefined`（不挂 usage 字段，ChatMessage 命中率为 null）。**验证**：① Vite build `✓ 3464 modules transformed, built in 25.23s`；② 新增 [test-v519-segment-bucketing.mjs](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/test-v519-segment-bucketing.mjs) **22 pass / 0 fail**（3 个问题都 round 0 / 1 个问题 4 轮 / Q1+Q2 互不干扰 / 没 usage fallback / 缺字段 fallback / assistant 在 user 前异常 / 4 个问题分别 round 0 / 段内 log 消息跳过共 8 case）；③ 全 backend 回归 test-agent-helpers 23 + test-convert-input-items 40 + test-f14-dispatch-integration 4 + test-route-apimode-dispatch 21 + test-v514-apimode-persistence 11 + test-v515-cached-tokens 9 + test-v517-hitrate-accumulation 20 = **128 pass / 0 fail**（不变）。**实操验证**：① Hard Refresh + 切到任一会话；② 看 asst1 / asst2 / asst3 各自的命中率**不再**互相覆盖；③ 切回**新问题**对话（之前不显示命中率的）— 应该也**正确**显示。**v5.19 反思教训**：v5.19 第一次实现用**单遍扫描**（在 user 处重置 segmentUsages）— 但**没**考虑 DB 消息顺序 assistant 在 usage **之前** → 第一遍扫到 assistant 时 segmentUsages 是空 → assistantUsages[id] = undefined。**应该**一开始就想清楚 DB 消息顺序 + 流式期间**只**创建一条 assistant 消息这两个事实。**v5.18 → v5.19 共 6 次修复**（v5.16 / v5.16b / v5.16c / v5.17 / v5.18 / v5.18b）— **5 次**在 SSE 流式路径修（最终 v5.18b 加路由层 res.write 修复），**1 次**在历史回看路径修（v5.19b 加 segment-bucket）。
  - **问题1**：实时页面还是没有显示，需要切换页面回来才能显示（**v5.16c** 的 React 18 batched ref 清空竞争**修复错位置**了 — 修的是 done 块末尾的 `if (data.usage)` 兜底，但 done 事件**本身**没带 `data.usage` 字段 — 这个块**永远**不进！**真正的根因**：v5.16 第一版 + v5.16b + v5.16c + v5.17 **全部**没真正工作 — **从来没有** `else if (data.type === 'usage')` 分支处理后端 SSE `type: "usage"` 事件（[responsesApi.js:238-251](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/services/responsesApi.js#L238-L251)），事件到达前端 onEvent 后**静默丢弃**，`roundUsagesRef.current` 永远 `{}` — done 累积 `_lastRoundUsage = null` → `usage: undefined` → 永远不显示。**切走再回来能看**：走历史回看路径（[App.jsx:391-407](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L391-L407)），`roundUsages` 从 DB messages 表**重新**构造（与 ref 无关）。
  - **问题2**：累积计算结果不对。第一个问题 1 轮 → 显示 99%；第二个问题 4 轮（round 0,1,2,3）→ 用户**期望**第二个问题只累积 0..3，**且不**影响第一个问题的 99%。**实际表现**：第二个问题后"上面的会变化"。**根因**：v5.17 累积代码本身逻辑**正确**（[App.jsx:1069-1092](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L1069-L1092)）— 但**因为**问题1 实时路径**没**挂上 usage（`roundUsagesRef` 永远空），用户看到的"第一个问题 99%"**只能**是**切走再回来**历史回看**复现**的 — 切回后第一个问题确实有 99%。第二个问题 SSE 期间**不**会改第一个问题（[App.jsx:1119-1122](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L1119-L1122) `findLastIndex` 找最后一条 assistant — 是**新**问题的 assistant，不改**旧**的），所以"上面的会变化"**可能**是用户**误解**（v5.18 修复后问题1 实时显示 + 第二个问题**独立**累积，行为应符合用户期望）。
  - **修法**（前端 1 文件，2 处）：① [App.jsx:1011-1031](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L1011-L1031) 在 `chunk` 分支后 / `reasoning_done` 分支前**新增** `else if (data.type === 'usage')` 分支 — 把 `data.usage` 存到 `roundUsagesRef.current[data.round]`（**关键**：v5.16 第一版只是把这段代码**放在 done 块末尾**作为 `if (data.usage)` 兜底 — 但 done 事件**不带** `data.usage`，兜底永远不进 — 应该**独立**成 `type: 'usage'` 事件分支）；② [App.jsx:1141-1144](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L1141-L1144) **删除** done 块末尾的 `if (data.usage)` 死代码（永远不进），改注释说明真正的处理已搬到 L1011-1031。**v5.17 累积逻辑保留**（0.._lastRound 累加）— 不动。**per-question 累积保证**：① done 块末尾 `roundUsagesRef.current = {}`（[App.jsx:1136](file:///d:/Ai_Program_Files/XTSQLQueryAgent/frontend/src/App.jsx#L1136)）保证**下一个问题**从空开始累加；② 每个问题 SSE 的 `currentRound` 编号**从 0 开始**（[responsesApi.js:411-412](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/services/responsesApi.js#L411-L412) `currentRound = maxToolCallsInitial - maxToolCalls` 初始 0，handler 每次启动重置）；③ setMessages `findLastIndex` 只改**最后一条** assistant，**不**改之前的（所以**第一个**问题的 final assistant 挂的 usage 字段**不会**被第二个问题覆盖）。**验证**：① Vite build `✓ 3464 modules transformed, built in 26.75s`；② 加 dev 模式 console.log（`window.location.hostname === 'localhost'`）让用户验证数据流 — usage 事件收到什么值、done 累积结果什么值；③ 全 backend 回归 108 pass / 0 fail（不变）。**实操验证步骤**：① Hard Refresh (Ctrl+Shift+R)；② 发新 query → 看 DevTools console 应**有** `[v5.18 debug] usage 事件 round=X: {...}` 日志（每个 LLM 调用 1 条）；③ 完成后看 `[v5.18 debug] done event: ... roundUsagesRef.current={...}` 日志；④ assistant 消息底部**应**立即看到"缓存命中率 XX%"（**不**用切走再回来）。**教训**（v5.18 反思）：v5.16 → v5.17 共 4 次修复**全错位置** — 修 done 块末尾的 `if (data.usage)` 兜底但兜底不进，应该**独立**成 `type: 'usage'` 事件分支。**根因**：v5.16 第一版就**没**加 `type: 'usage'` 事件分支，错误地把代码**放在** done 块末尾作为"兜底"，但 done 事件不带 usage 字段，兜底永远不进。**应该**一开始就**独立**成 `else if (data.type === 'usage')`。
...
| 2026-08-07 | v5.1 | agent | **Step 1 实施完成**：在 [llm.js:340-349](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/services/llm.js#L340-L349) 插入 `setLastMessages(messages)` setter，深拷贝实现 `lastMessages = JSON.parse(JSON.stringify(messages))`（与 CC path L1558 1:1 对齐）+ 6 行注释说明深拷贝原因。**验证**：① `node --check` exit 0；② 实际 import 确认 setLastMessages 是 named export（total exports 10 → 11）；③ 深拷贝行为测试 PASS（修改原数组后 snapshot 仍为 `[{role:'user',content:'hello'}]`，length=1）；④ 跑相关测试 4 个文件全通过（test-route-apimode-dispatch 21 pass / test-llm-config-apimode 22 pass / test-llm-timeout 22 pass / test-favorite-query 74 pass，共 **139 pass, 0 fail**）。|
| 2026-08-07 | v5.2 | agent | **Step 2 实施完成 + 1 项偏差修订**：① **Step 2.1** 在 llm.js 给 7 个底层依赖函数加 export 关键词（纯加法 0 实现改动）：`fixBareQuotesInJsonArgs` L54 / `flushLogs` L317 / `getOrCreateRegistry` L358 / `checkAndFilterDuplicateCall` L495 / `recordToolCall` L648 / `queueLog` L878 / `saveMessagesToDb` L912（首次 SearchReplace 因 caching 问题导致 3 个 export 未生效，验证发现后重做：flushLogs / getOrCreateRegistry / checkAndFilterDuplicateCall 各重 SearchReplace 一次，最终 total exports 11 → **18**）。② **偏差修订**：发现原计划 [Step 2 import 列表](file:///d:/Ai_Program_Files/XTSQLQueryAgent/.trae/documents/phase-2-responses-api-implementation.md#L93-L101) 只列 4 个函数（`loadMessagesFromDb` / `saveMessagesToDb` / `getOrCreateRegistry` / `setLastMessages`），实际 `executeToolCallsInStages` 还需要 5 个（`fixBareQuotesInJsonArgs` / `checkAndFilterDuplicateCall` / `recordToolCall` / `queueLog` / `flushLogs`）。用户决策：加 7 个 export 到 llm.js（方案 A，纯加法）。③ **Step 2.2** 创建 [agentHelpers.js](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/src/services/agentHelpers.js)（~470 行，5 个 helper：initMessagesForRun / getPrunedToolsForRun / saveRunState / recordPendingUserChoices / executeToolCallsInStages）。**executeToolCallsInStages 不在内部调 saveRunState / recordPendingUserChoices**（避免重复写盘 + 便于 caller 控制时机）。④ **Step 2.3** 行为测试：[test-agent-helpers.mjs](file:///d:/Ai_Program_Files/XTSQLQueryAgent/backend/test-agent-helpers.mjs) 16 case 全 pass（initMessagesForRun 3 / getPrunedToolsForRun 1 / saveRunState 3 含深拷贝验证 / recordPendingUserChoices 3 含 MAX_USER_CHOICE_PER_TURN 边界 / executeToolCallsInStages 6 含成功+工具不存在+参数解析失败错误路径）。⑤ **Step 2.4** 回归：现有 4 个相关测试全通过（**139 pass / 0 fail**）。|
